...
image = cv2.imread('images/maze.png')
gray_image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
# cnts = cv2.get_contours(gray_image, 200)
# cnt = sorted(cnts, key=cv2.contourArea, reverse=True)[0]

thresholded_image = cv2.adaptive_threshold(gray_image, cv2.THRESH_BINARY_INV)
cv2.imshow("Output", cv2.image_resize(thresholded_image, height=600))
cv2.waitKey()
...
cv2.imshow("Output", cv2.image_resize(solution_image, height=600))
cv2.waitKey()
...
cv2.imwrite("images/result.png", solution_image)
